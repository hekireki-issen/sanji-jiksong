...
public class OrderService {
    ...
    public OrderResponse createOrder(User user, OrderRequest request) {
            ...
            for (OrderRequest.OrderListRequest itemReq : request.orderLists()) {
                Item item = itemRepository.findById(itemReq.itemId())
                        .orElseThrow(() -> new IllegalArgumentException("존재하지 않는 상품입니다.")); // 임시 예외
    ...
    // 주문 수정
    @Transactional
    public OrderResponse updateOrderItem(Long orderId, Long itemId, int newCount, User user) {
        Order order = orderRepository.findById(orderId)
                .orElseThrow(OrderException.OrderNotFoundException::new);
        ...
        OrderList target = order.getOrderLists().stream()
                .filter(ol -> ol.getItem().getId().equals(itemId))
                .findFirst()
        ...
        target.updateCount(newCount); // count, countPrice 자동 계산됨
